...
public:
    vector<int> lexicographicallySmallestArray(vector<int>& nums, int limit) {
        ...
        unordered_map<int, list<int>> groupToList; // taking list because the time complex. of vector in push and pop is 0(n) so it will give TLE so instead of vector, use list.
        ...
        for(int i = 0; i < n; i++) {
            ...
            groupToList[group].pop_front(); //0(1)
        }
